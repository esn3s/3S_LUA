...
function hitbox_load(obj, i, typeHb, facing_dir, offset_x, offset_y, addr, type)
	...
	if fba and typeHb == HITBOX_ACTIVE then
	...
	if typeHb == HITBOX_THROWABLE then
		obj.ta_hboxes[i] = {
			l_orig = l_orig,
			r_orig = r_orig,
			t_orig = t_orig,
...
function update_game_object(obj, base, type)
	...
	for i = 1, 4 do
		hitbox_load(obj, i, HITBOX_ACTIVE, obj.facing_dir, obj.pos_x, obj.pos_y, a_hb_addr, type)
