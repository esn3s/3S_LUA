...
function keyDisplay(image_l1, image_l2, image_m1, image_m2, image_h1, image_h2, image_s1, image_s2, x, y, buttonAddress,startAddress)
	...
	input_lk = bitReturn(memory.readbyte(buttonAddress-1),0)
	input_mk = bitReturn(memory.readbyte(buttonAddress-1),1)
	input_hk = bitReturn(memory.readbyte(buttonAddress-1),2)
...
function training()
	...
	drawOriginNum(nums0, offsetX+124, offsetY, memory.readbyte(0x02010D61) + 10, 3, 10)
	offsetY = offsetY + 20
...
function c_cube_script()
	--************************************************************
	--************************************************************
	--************************************************************
	--ここにフレーム処理の直前に実行させたい処理を書いてください
	--（次フレームでの入力の取得・設定、メモリへの書き込みなど）
	--************************************************************
	--************************************************************
	--************************************************************
	...
	--BGM音量
	--memory.writebyte(0x02078D06,0x80)
	...
	--memory.writebyte(0x02011387,0x0F)
	...
	--memory.writebyte(0x020691A3,160)
	...
	or memory.readbyte(0x020154A7) == 3 or  memory.readbyte(0x020154A7) == 7 or  memory.readbyte(0x020154A7) == 8 then
		...
		if cheatModeNum == 0 then
			...
			if inputView == 1 then
				...
				--キーディスプレイその１（１P側）
				keyDisplay(button_l1, button_l2, button_m1, button_m2, button_h1, button_h2, button_s1, button_s2, 40, 184, 0x0202564B, 0x206AA8C)
					...
					if memory.readbyte(0x020154D3) == 2 then
						if memory.readbyte(0x02069520) ~= 0 then
							...
							if bDenjinStarted == true then
								...
								gui.text(100, 50, sDenjin, 0x80FFFFFF, 0x000000FF)
							end
						end
