...
--・通常の食らい判定＆攻撃判定だけ表示していた			→	攻撃を出した時の食らい判定も表示するように変更
...
local HITBOX_PASSIVE_COLOUR = 0x00008040
local HITBOX_ACTIVE_COLOUR  = 0x00FF0040
local HITBOX_PASSIVE2_COLOUR = 0x0000FF40
...
function render_sfiii_hitboxes()
	...
	--対戦中でなければ終了
	if memory.readword(0x020154A6) ~= GAME_PHASE_PLAYING then
...
----ここから下はぷらすが作った関数群。このluaでは殆ど使ってない----
...
gui.register( function()
		...
		--ここ以降に、攻撃判定のデータが眠っているっぽい。
		--見たい場合はここのコメントを外す
		viewMemory(0x02068F30)
